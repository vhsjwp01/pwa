...
apt update
apt install -y inxi coreutils git ntp ntpdate iw gnupg gnupg2 gnupg1 make openssh-server screen curl uuid-runtime hostapd wget rsync ifupdown net-tools jq htop iftop iotop vim bc ethtool bridge-utils wireless-tools firmware-realtek firmware-ralink firmware-iwlwifi 
